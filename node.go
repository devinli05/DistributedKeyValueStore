...
func handleRequestUDPHelper(packet *udpComm, retAddr *net.UDPAddr) {
	...
	if packet.Type == "Get" {
		...
		} else if strings.EqualFold(packet.Status, "Retrieve") {
			retVal := Getudp(packet)
			msg := "Get " + retVal.Key + ":" + retVal.Val
			outBuf := addGovecLog(msg, retVal)
			udpConn.WriteToUDP(outBuf, retAddr)
		} else {
			retVal := distribute(packet, "Get")
...
func distribute(packet *udpComm, packet_type string) *udpComm {
	...
	case strings.EqualFold(packet_type, "Get"):
		msg = Logger.PrepareSend("Sending Get request for Key: "+get.Key+" and Val: "+get.Val, get)
	case strings.EqualFold(packet_type, "Remove"):
...
func contactNodes() {
	...
	for _, j := range requestBuffer {
		...
		if err == nil {
			...
			LogMutex.Lock()
			Logger.UnpackReceive("Receive Message", buffer[:bytesRead], &packet)
			Logger.LogLocalEvent("Received Keys: " + strings.Join(packet.Keys, " ") + " Received Values: " + strings.Join(packet.Values, " "))
			LogMutex.Unlock()
			fmt.Println(packet)
			kvMutex.Lock()
...
func contactMyReplicas() {
	...
	for _, j := range requestBuffer {
		...
		if err == nil {
			...
			Logger.LogLocalEvent("Keys Received: " + strings.Join(packet.Keys, " ") + " Values Received: " + strings.Join(packet.Values, " "))
			LogMutex.Unlock()
			fmt.Println(packet)
			kvMutex.Lock()
